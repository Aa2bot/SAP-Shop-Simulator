...
function detectEffect(text) {
    ...
    if (lower.includes('free roll'))
        return { effect: 'free_roll_handler' };
    if (lower.includes('roll'))
        return { effect: 'roll_based_trigger_handler' };
    if (lower.includes('trumpet'))
        return { effect: 'trumpet_handler' };
    if (lower.includes('toy'))
        return { effect: 'toy_break_handler' };
    if (lower.includes('perk') || lower.includes('ailment'))
        return { effect: 'perks_ailments_handler' };
    if (lower.includes('summon'))
        return { effect: 'summon_pet', target: 'self' };
...
export function buildAbilityTable() {
    ...
    for (const pack of Object.values(packs)) {
        ...
        for (const pet of pets) {
            ...
            const abilityText = pet?.ability?.level1 ?? '';
