...
const HEART_ICON = '/assets/art/Public/Icons/TextMap-resources.assets-31-split/heart.png';
const FIST_ICON = '/assets/art/Public/Icons/TextMap-resources.assets-31-split/fist.png';
const ICE_ICON = '/assets/art/Public/Icons/Icecube.png';
const UI_SOUNDS = {
  click: 'click',
  freeze: 'freeze',
  unfreeze: 'unfreeze',
...
const SUPPLEMENTAL_TEXTURE_LOOKUP = {
  bilby: '/assets/art/Public/Pets/Bilby.png',
  lemming: '/assets/art/Public/Pets/Lemming.png',
  purplefrog: '/assets/art/Public/Pets/PurpleFrog.png',
  lime: '/assets/art/Public/Food/Lime.png',
  squash: '/assets/art/Public/Food/Squash.png',
  bokchoy: '/assets/art/Public/Food/BokChoy.png',
  honeydewmelon: '/assets/art/Public/Food/HoneydewMelon.png',
  pumpkin: '/assets/art/Public/Food/Pumpkin.png'
};
...
const state = {
  ...
  choiceMode: 'toy',
  foodChoices: [],
  foodChoiceContext: null,
  foodChoiceRemaining: 0,
  battleReport: null,
...
function startFoodChoiceFlow({ choices = [], remaining = 1, title = 'Pick Food', context = '' } = {}) {
  state.choiceMode = 'food';
  state.foodChoices = [...choices];
  state.foodChoiceRemaining = Math.max(1, Number(remaining || 1));
  state.foodChoiceContext = { title, context };
  renderToyChoiceDisplay();
  if (choices.length <= 1) {
    const pick = choices[0];
    while (pick && state.foodChoiceRemaining > 0) {
      selectFoodChoice(pick.name);
    }
    return;
  }
  openToyPickerModal({ mode: 'food' });
}

function selectFoodChoice(foodName) {
  const choice = (state.foodChoices || []).find((f) => normalizeName(f.name) === normalizeName(foodName));
  if (!choice) return;
  const remaining = Math.max(0, Number(state.foodChoiceRemaining || 0));
  if (remaining <= 0) {
    closeToyPickerModal(false);
    return;
  }
  ...
  state.foodChoiceRemaining = Math.max(0, remaining - 1);
  renderShopFoods();
  renderShopPets();
  const left = state.foodChoiceRemaining;
  if (left > 0) {
    state.foodChoiceContext = {
      ...(state.foodChoiceContext || {}),
      context: `Choose ${left} item(s) to stock.`
    };
  }
  setStatus(`Stocked ${choice.name}${left > 0 ? ` (${left} remaining)` : ''}.`);
  if (left <= 0) {
    closeToyPickerModal(false);
  } else {
    renderToyChoiceDisplay();
...
function renderToyChoiceDisplay() {
  ...
  const isFoodMode = state.choiceMode === 'food';
  toyChoiceSlots.innerHTML = '';
  const choices = Array.isArray(isFoodMode ? state.foodChoices : state.toyChoices)
    ? (isFoodMode ? state.foodChoices : state.toyChoices)
    : [];
  if (!choices.length) {
    if (toyPickerContext) {
      toyPickerContext.textContent = isFoodMode
        ? 'No food choices available.'
        : 'No toy choices currently available.';
    }
    return;
  }
  if (toyPickerTitle) {
    toyPickerTitle.textContent = isFoodMode
      ? (state.foodChoiceContext?.title || 'Pick Food')
      : 'Pick Your Toy';
  }
  if (toyPickerContext) {
    if (isFoodMode) {
      const remaining = Math.max(0, Number(state.foodChoiceRemaining || 0));
      toyPickerContext.textContent = state.foodChoiceContext?.context
        || `Choose ${remaining || 1} item(s) to stock.`;
    } else {
  ...
  choices.forEach((choice) => {
    ...
    if (!isFoodMode && normalizeName(state.playerToy) === normalizeName(choice.name)) {
      card.classList.add('selected');
    }
    const view = isFoodMode
      ? {
          name: choice.name,
          tier: choice.tier,
          ability: choice.ability,
          path: resolveTexture(choice.name)
        }
      : {
          name: choice.name,
          tier: choice.tier,
    ...
    bindHoverTooltip(card, isFoodMode ? 'food' : 'toy', view);
    const media = renderCardImage(view);
    const label = document.createElement('div');
    label.className = 'name';
    label.textContent = `${choice.name} (T${choice.tier})`;
    const btn = document.createElement('button');
    btn.className = 'toy-choice-use';
    btn.type = 'button';
    btn.textContent = isFoodMode ? 'Stock' : 'Use';
    btn.addEventListener('click', () => {
      if (isFoodMode) {
        selectFoodChoice(choice.name);
      } else {
        selectToyChoice(choice.name);
      }
    });
    card.tabIndex = 0;
...
function closeToyPickerModal(skipSelection = true) {
  ...
  if (isFoodMode) {
    state.foodChoices = [];
    state.foodChoiceContext = null;
    state.foodChoiceRemaining = 0;
  } else {
...
function queueNextTurnGold(amount) {
  ...
  setStatus(`Queued +${add} gold next turn (total +${state.nextTurnBonusGold}).`);
  return add;
}

function resolveTurnStartGold(bonus = state.nextTurnBonusGold) {
  const storedBonus = Math.max(0, Number(bonus || 0));
  return START_GOLD + storedBonus;
}
...
function applySpecialFoodEffect(food, targetIdx, foodRef) {
  ...
  if (/Give the two left-most shop pets \+(\d+) attack and \+(\d+) health/i.test(ability)) {
    const m = ability.match(/Give the two left-most shop pets \+(\d+) attack and \+(\d+) health/i);
    const atk = Number(m?.[1] || 0);
    const hp = Number(m?.[2] || 0);
    let applied = 0;
    (state.shopPets || []).some((slot) => {
      if (!slot?.item) return false;
      slot.item.baseAttack = Math.max(1, Number(slot.item.baseAttack || 1) + atk);
      slot.item.baseHealth = Math.max(1, Number(slot.item.baseHealth || 1) + hp);
      markPetSlotFx(slot, 'stock');
      applied += 1;
      return applied >= 2;
    });
    renderShopPets();
    return { ok: true, note: `${food.name}: buffed ${applied} left-most shop pet(s)` };
  }
  ...
  if (/Choose one tier 5-6 perk food to stock for free/i.test(ability)) {
    const pool = getAllFoodsAcrossPacks().filter((f) => {
      const tier = Number(f.tier || 1);
      return tier >= 5 && tier <= 6 && /perk/i.test(String(f.ability || ''));
    });
    if (!pool.length) return { ok: false, note: `${food.name}: no tier 5-6 perk foods available.` };
    const charges = Math.max(1, Math.min(3, Number(pet?.levelInt || pet?.levelValue || 1)));
    startFoodChoiceFlow({
      choices: pool,
      remaining: charges,
      title: `${food.name} - Pick Perk Food`,
      context: `Choose ${charges} perk food(s) (Tier 5-6) to stock for free.`
    });
    return { ok: true, note: `${food.name}: awaiting perk food selection` };
  }
  ...
  if (/Choose one free tier 6 food to stock from ALL packs/i.test(ability)) {
    ...
    if (!pool.length) return { ok: false, note: `${food.name}: no tier 6 foods available.` };
    const charges = Math.max(1, Math.min(3, Number(pet?.levelInt || pet?.levelValue || 1)));
    startFoodChoiceFlow({
      choices: pool,
      remaining: charges,
      title: `${food.name} - Pick Tier 6 Food`,
      context: `Choose ${charges} Tier 6 food(s) from any pack to stock for free.`
    });
    return { ok: true, note: `${food.name}: awaiting Tier 6 food selection` };
  }
...
async function endTurn() {
  ...
  applyTurnTierFilters();
  refillShop(true);
  state.gold = resolveTurnStartGold(bonusGold);
  state.nextTurnBonusGold = 0;
...
async function loadPackData() {
  // Updated path: data file relocated to assets/data/ to align with new registries.
  const res = await fetch('/assets/data/pack-data.json');
  if (!res.ok) {
...
window.addEventListener('keydown', (ev) => {
  ...
  const choices = Array.isArray(state.choiceMode === 'food' ? state.foodChoices : state.toyChoices)
    ? (state.choiceMode === 'food' ? state.foodChoices : state.toyChoices)
    : [];
  ...
  if (ev.key === 'Enter') {
    ...
    if (selected?.name) {
      if (state.choiceMode === 'food') {
        selectFoodChoice(selected.name);
      } else {
